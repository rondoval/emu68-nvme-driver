...
/*
 * nvme_get_error_status_str - translate a raw NVMe completion status to a string
 *
 * Maps the status code from an NVMe Completion Queue Entry (CQE) to a
 * human-readable description for use in kernel log messages.  The status
 * field encodes both the Status Code Type and the Status Code; only the
 * combined SCT+SC bits are used as an index into nvme_statuses[].
 *
 * Called from nvme_log_error() and other error-reporting paths throughout
 * the NVMe host driver whenever a command completes with a non-success status.
 *
 * @status: raw 16-bit status field from the CQE (DNR/More bits are masked off)
 * Returns: a pointer to a constant string describing the status, or "Unknown"
 *          if the code is outside the table or has no entry.
 */
...
/*
 * nvme_get_opcode_str - translate an NVMe I/O command opcode to a string
 *
 * Maps an NVMe I/O submission queue opcode (e.g. Read, Write, Flush) to its
 * human-readable name for error messages and tracing.  I/O opcodes live in a
 * separate namespace from admin opcodes; use nvme_get_admin_opcode_str() for
 * commands sent on the admin queue.
 *
 * Exported so that transport drivers (pcie, rdma, tcp, fc) can call it when
 * logging failed or timed-out I/O commands.
 *
 * @opcode: 8-bit opcode from the NVMe I/O Submission Queue Entry
 * Returns: a pointer to a constant string naming the opcode, or "Unknown"
 *          if the opcode is outside the table or has no entry.
 */
...
/*
 * nvme_get_admin_opcode_str - translate an NVMe admin command opcode to a string
 *
 * Maps an NVMe Admin Queue opcode (e.g. Identify, Set Features, Format NVM)
 * to its human-readable name.  Admin opcodes occupy a different opcode space
 * than I/O opcodes and cover device-management operations rather than data
 * transfer; use nvme_get_opcode_str() for I/O queue commands instead.
 *
 * Exported so that transport drivers can log failed admin commands with a
 * descriptive opcode name rather than a bare hex value.
 *
 * @opcode: 8-bit opcode from the NVMe Admin Submission Queue Entry
 * Returns: a pointer to a constant string naming the opcode, or "Unknown"
 *          if the opcode is outside the table or has no entry.
 */
