...
static void nvme_end_req(struct nvme_request *req)
{
	...
	KprintfH("[nvme] end_req: req=%lx tag=%lu status=0x%lx error=%ld done=%lx waiter=%lx io=%lx\n",
...
{
	KprintfH("[nvme] complete_rq: req=%lx tag=%lu opcode=0x%02lx status=0x%lx unit=%lx ctx=%lx\n",
			 (ULONG)req, (ULONG)req->tag,
			 (ULONG)req->cmd.common.opcode,
			 (ULONG)req->status, (ULONG)req->unit, (ULONG)req->ctx);
